...
@llm.ai_callable()
async def discounted_rate(current_rate:float) -> float:
...
@llm.ai_callable() 
async def retention_rate(counter_rate:Annotated[
...
@llm.ai_callable() 
async def create_ticket(title:Annotated[
...
@llm.ai_callable()
async def send_documents_to_sign(email: Annotated[
...
@llm.ai_callable()
async def transfer_human_agent() -> str: 
...
"""
    ...
    functions=[fetch_posted_rates, discounted_rate, retention_rate, create_ticket ],
)