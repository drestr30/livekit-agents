...
class AssistantFnc(llm.FunctionContext):
    ...
    @llm.ai_callable()
    async def fetch_posted_rates(self):
    ...
    @llm.ai_callable()
    async def discounted_rate(self, current_rate:Annotated[
            float, llm.TypeInfo(description="This is the curent rate provided to the customer for his mortgage renewal.")
        ]) -> str:
...
renewal_prompt = """As a Mortgage Renewal Specialist, you are tasked with facilitating mortgage renewals whenever the primary assistant delegates the task to you.
...
2. **Discuss Available Rates:**
   ...
   - Discuss and present the current posted mortgage rate that match the terms of the customer's existing mortgage. Dont present all the rates, only the one that applies.
...
def run_multimodal_agent(ctx: JobContext, participant: rtc.RemoteParticipant):
    ...
    model = openai.realtime.RealtimeModel().with_azure(
        ...
        api_version=os.environ['AZURE_OPENAI_CHAT_DEPLOYMENT_VERSION'],
        instructions=(renewal_prompt),
